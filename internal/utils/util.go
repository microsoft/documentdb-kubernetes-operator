...
func GetDocumentDBServiceDefinition(documentdb *dbpreview.DocumentDB, replicationContext *ReplicationContext, namespace string, serviceType corev1.ServiceType) *corev1.Service {
	...
	if documentdb.Spec.AzureDNSPrefix != "" && replicationContext.IsPrimary() {
		serviceSpec.ObjectMeta.Annotations = map[string]string{
			"service.beta.kubernetes.io/azure-dns-label-name": documentdb.Spec.AzureDNSPrefix,
		}
	}
