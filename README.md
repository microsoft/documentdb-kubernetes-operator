...
```sh
kubectl apply -f operator/scripts/deployment-examples/mongosh-client.yaml
```
...
cd documentdb-kubernetes-operator/operator/scripts/test-scripts
...
# Build the operator (from the operator directory)
cd operator
make build
