...
RG=german-aks-fleet-rg
MEMBERS=$(az aks list -g "$RG" -o json | jq -r '.[] | select(.name|startswith("member-")) | .name')
echo -e "Members:\n$MEMBERS"

# Ensure contexts
for C in $MEMBERS; do
  echo "Fetching creds for $C..."
  az aks get-credentials -g "$RG" -n "$C" --overwrite-existing 1>/dev/null || true
done
