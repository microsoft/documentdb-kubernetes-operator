...
# Strategy:
#  - Prefer k-hub alias context if available and functional.
...
# Load any aliases/environment the deploy script may have added
if [ -f "$HOME/.bashrc" ]; then
  # shellcheck disable=SC1090
  source "$HOME/.bashrc" || true
fi
HUB_CONTEXT=$(k-hub config current-context 2>/dev/null || true)

if [ -z "$HUB_CONTEXT"]; then
    HUB_CONTEXT="hub"
    echo "k-hub failed?! - Using hub context: $HUB_CONTEXT"
fi
